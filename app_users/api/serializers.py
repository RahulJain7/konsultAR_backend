...
class AppUserRegisterSerializer(serializers.ModelSerializer):
    ...
    role = serializers.CharField(style={'input_type': 'choice'})
    # password = serializers.CharField(style={'input_type': 'password'}, write_only=True)
    ...
    def validate_email(self, value):
        user_qs = User.objects.filter(email__iexact=value)
        AppUser_qs = AppUser.objects.filter(email__iexact=value)
    ...
    def validate_mobile(self, value):
        user_qs = User.objects.filter(email__iexact=value)
        AppUser_qs = AppUser.objects.filter(mobile=value)
    ...
    def create(self, validated_data):
        ...
        print (user)
        AppUser_obj = AppUser.objects.create(user=user,**validated_data)
        return AppUser_obj
